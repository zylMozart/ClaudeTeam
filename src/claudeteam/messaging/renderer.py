...
def strip_runtime_commands(text: str) -> str:
    ...
    for line in str(text or "").splitlines():
        cleaned = RUNTIME_CMD_RE.sub("", line).strip()
        if cleaned:
...
def truncate_long(text: str, limit: int) -> str:
    text = str(text or "")
    if len(text) <= limit:
        return text
