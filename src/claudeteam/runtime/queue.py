#!/usr/bin/env python3
"""Message queue management helpers for ClaudeTeam runtime."""
...
def _pending_dir():
    """Resolve queue directory with runtime env overrides.

    Priority:
      1) CLAUDETEAM_PENDING_DIR (explicit queue override)
      2) CLAUDETEAM_STATE_DIR/queue/pending_msgs (no-live/runtime-isolated path)
      3) legacy PROJECT_ROOT/workspace/shared/.pending_msgs
    """
    pending_dir = os.environ.get("CLAUDETEAM_PENDING_DIR", "").strip()
    if pending_dir:
        return pending_dir
    state_dir = os.environ.get("CLAUDETEAM_STATE_DIR", "").strip()
    if state_dir:
        return os.path.join(state_dir, "queue", "pending_msgs")
    return PENDING_DIR
...
def has_pending_messages(agent_name):
    ...
    with _queue_lock:
        queue_file = os.path.join(_pending_dir(), f"{agent_name}.json")
...
def check_manager_unread(last_check_time):
    ...
    queue_file = os.path.join(_pending_dir(), "manager.json")
