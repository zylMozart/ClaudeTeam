...
| Slash dispatcher | `slash_commands.dispatch(text) -> (matched, reply)` | `scripts/slash_commands.py` | Preserve tuple contract and reply forms: `str` or `{"text": ..., "card": ...}`. | Future unit tests calling dispatch without tmux/Feishu. |
| tmux injection | `inject_when_idle(session, window, text, ...) -> InjectionResult` | `scripts/tmux_utils.py` | Preserve bool compatibility, `error`, `target`, `unsafe_input`, `busy_before`, `submitted`, `residual_visible`. | Future unit tests with subprocess mocked. |
| CLI adapter shell bridge | `python3 scripts/cli_adapters/resolve.py <agent> <attr> ...` | `scripts/cli_adapters/resolve.py` | Preserve `spawn_cmd`, `resume_cmd`, `ready_markers`, `busy_markers`, `process_name`, `thinking_init_hint`. | Static wrapper existence; future fake team.json tests. |
