...
- Router 通过 lark-cli WebSocket 实时接收群消息，自动路由到对应 Agent
...
- 群聊图片 → `workspace/shared/images/`（Router 通过 lark-cli 自动下载）
...
## 8. 底层架构（Agent 无需操心，但需知晓）

- 所有飞书 API 操作通过 `lark-cli`（`@larksuite/cli`）执行，认证由 lark-cli 统一管理
- 群消息通过 lark-cli WebSocket 长连接实时推送到 Router，不再轮询
- Agent 只需使用 `feishu_msg.py` 的 CLI 命令，底层实现透明
