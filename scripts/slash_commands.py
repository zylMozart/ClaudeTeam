...
def _build_server_load_text(data: dict, now: str) -> str:
    lines = [f"🖥️ /server-load — {_hostname()} ({now} 北京时间)\n"]
    host = data["host"]
    if host["cpu"]:
        c = host["cpu"]
...
def _cmd_server_load(text: str):
    if not re.fullmatch(r"/server-load\s*", text):
        return None
...
# ── dispatch ───────────────────────────────────────────────────
_HANDLERS = [
    _cmd_help, _cmd_team, _cmd_usage, _cmd_server_load, _cmd_tmux,
    _cmd_send, _cmd_compact,
]
