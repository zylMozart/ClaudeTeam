...
def assert_invariants(name: str, target: str, rendered: str):
    ...
    if name.startswith("chinese_curly_quote"):
        ...
        if target != "split":
            assert "好，把5秒抓10次的" in rendered or "前 N 轮" in rendered, name
...
def check_split_regression():
    split_cases = {
        ...
        "chinese_curly_quote_long": CASES["chinese_curly_quote_long"],
    }
    for name, body in split_cases.items():
        ...
        if name == "chinese_curly_quote_long":
            assert len(body) >= 1500, (name, len(body))
            assert "前 N 轮" in joined, name
            assert joined.count("\u201c") >= 40, (name, joined.count("\u201c"))
            assert joined.count("\u201d") >= 40, (name, joined.count("\u201d"))
...
def check_delivery_card_splitting():
    ...
    try:
        ...
        captured.clear()
        feishu_msg.cmd_say("toolsmith", CASES["chinese_curly_quote_short"])
        assert len(captured) == 1, "short curly-quote message must fit one card"
        joined_short = _card_markdown(captured[0])
        assert CASES["chinese_curly_quote_short"] in joined_short, "curly-quote short body altered"

        captured.clear()
        feishu_msg.cmd_say("toolsmith", CASES["chinese_curly_quote_long"])
        joined_long = "\n".join(_card_markdown(c) for c in captured)
        assert "前 N 轮" in joined_long, "curly-quote long body lost header"
        assert joined_long.count("\u201c") >= 40, joined_long.count("\u201c")
        assert joined_long.count("\u201d") >= 40, joined_long.count("\u201d")
        assert not any(m in joined_long for m in FORBIDDEN_VISIBLE_MARKERS), "curly-quote long body truncated"
    finally:
