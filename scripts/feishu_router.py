...
from config import (
    AGENTS, TMUX_SESSION, PROJECT_ROOT, load_runtime_config, LARK_CLI,
)
...
from claudeteam.runtime.paths import ensure_parent, legacy_script_state_file, runtime_state_file
import tmux_command
import team_command
...
def wake_agent(agent_name, message_preview, sender_agent=None,
    ...
    if sender_agent:
        ...
        if len(content) > 400:
            msg_file = os.path.join(ROUTER_MSG_DIR,
...
def handle_event(event):
    ...
    if matched:
        ...
        print(line)
        try:
            ensure_parent(TMUX_INTERCEPT_LOG)
            with open(TMUX_INTERCEPT_LOG, "a") as f:
                f.write(line + "\n")
        except Exception:
            pass
        try:
...
def _load_cursor():
    ...
    for path in (CURSOR_FILE, LEGACY_CURSOR_FILE):
        try:
            with open(path) as f:
...
def _pid_file_is_live_router(path):
